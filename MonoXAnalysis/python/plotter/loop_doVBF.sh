...
lumi="24.5"
storeDir="VBF_shape_study_24p5fb_new"         # name of directory containing all the tests you would do. You can specify the luminosity used in the name
exclude_cut=" -X ICHEP_data "          # I was always using ICHEP data, -X ICHEP_data remove this cut to use all available dataset
#exclude_cut=" "
...
# for the file common_plots.txt that defines the binning for mjj and detajj, in order to do a loop changing selection on those 
# variables (and considering I can't have empty bins in the fit) I decided to copy it in vbfdm/common_plots_txt many times depending on the shape variables' cut
# folder and each time copy the copied file with the right binning into common_plots.txt, which is backuped in vbfdm/common_plots_txt

uptothiscut="deta2j"
var="mjj"    # mjj or detajj (or dphijm, but not for now)

mjjcut=(500 500 500 500 1000 1000 1000 1000 500 500 500 500 1000 1000 1000 1000)

detajjcut=(3.0 3.5 3.0 3.5 3.0 3.5 3.0 3.5 3.0 3.5 3.0 3.5 3.0 3.5 3.0 3.5)
detajjcut2=(3p0 3p5 3p0 3p5 3p0 3p5 3p0 3p5 3p0 3p5 3p0 3p5 3p0 3p5 3p0 3p5)

#dphijmcut=(2.0 2.0 2.3 2.3 2.0 2.0 2.3 2.3)
#dphijmcut2=(2p0 2p0 2p3 2p3 2p0 2p0 2p3 2p3)

dphijmcut=(1.0 1.0 1.5 1.5 1.0 1.0 1.5 1.5 2.0 2.0 2.3 2.3 2.0 2.0 2.3 2.3)
dphijmcut2=(1p0 1p0 1p5 1p5 1p0 1p0 1p5 1p5 2p0 2p0 2p3 2p3 2p0 2p0 2p3 2p3)


nrun_plus1="${#dphijmcut[@]}"
let "nrun=${nrun_plus1}-1"
...
#get proper file with histogram binning
    ...
    if [ "$var" = "mjj" ]; then
	comm_plots_file="vbfdm/common_plots_txt/${var}_${var}${mjjcut[$i]}.txt"
    elif [ "$var" = "detajj" ]; then
	comm_plots_file="vbfdm/common_plots_txt/${var}_${var}${detajjcut2[$i]}.txt"
    else 
    ...
    fitdir="${var}_mjj${mjjcut[$i]}_detajj${detajjcut2[$i]}_dphijm${dphijmcut2[$i]}"  # name of dir that contains the output
...
#analysis_changeCut=" -R mass2j mass2j 'mass_2(JetClean1_pt,JetClean1_eta,JetClean1_phi,0.,JetClean2_pt,JetClean2_eta,JetClean2_phi,0.) > 500' -R deta2j deta2j 'abs(JetClean1_eta-JetClean2_eta) > 3.5' "
    ...
    analysis_changeCut=" -R dphijm dphijm 'abs(dphijm) > ${dphijmcut[$i]} && abs(dphijm) < 3.15' -R deta2j deta2j 'abs(JetClean1_eta-JetClean2_eta) > ${detajjcut[$i]}' -R mass2j mass2j 'mass_2(JetClean1_pt,JetClean1_eta,JetClean1_phi,0.,JetClean2_pt,JetClean2_eta,JetClean2_phi,0.) > ${mjjcut[$i]}' "
    analysis_excludeCut="${exclude_cut}"
